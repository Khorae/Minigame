...
public class ServerboundEditBookPacket implements Packet<ServerGamePacketListener> {
    ...
    public ServerboundEditBookPacket(FriendlyByteBuf buf) {
        ...
        this.pages = buf.readCollection(FriendlyByteBuf.limitValue(Lists::newArrayListWithCapacity, 200), buf2 -> buf2.readUtf(8192));
        this.title = buf.readOptional(buf2 -> buf2.readUtf(128));
    }

    @Override
    public void write(FriendlyByteBuf buf) {
        buf.writeVarInt(this.slot);
        buf.writeCollection(this.pages, (buf2, page) -> buf2.writeUtf(page, 8192));
        buf.writeOptional(this.title, (buf2, title) -> buf2.writeUtf(title, 128));
    }
