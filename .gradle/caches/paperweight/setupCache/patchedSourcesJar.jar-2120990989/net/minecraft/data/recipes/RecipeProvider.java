...
public abstract class RecipeProvider implements DataProvider {
            ...
            new RecipeOutput() {
                ...
                public void accept(ResourceLocation recipeId, Recipe<?> recipe, @Nullable AdvancementHolder advancement) {
                    if (!set.add(recipeId)) {
                        ...
                        list.add(DataProvider.saveStable(writer, Recipe.CODEC, recipe, RecipeProvider.this.recipePathProvider.json(recipeId)));
                        if (advancement != null) {
                            list.add(
                                DataProvider.saveStable(
                                    writer, Advancement.CODEC, advancement.value(), RecipeProvider.this.advancementPathProvider.json(advancement.id())
                                )
                            );
                        }
                    }
    ...
    protected CompletableFuture<?> buildAdvancement(CachedOutput cache, AdvancementHolder advancement) {
        return DataProvider.saveStable(cache, Advancement.CODEC, advancement.value(), this.advancementPathProvider.json(advancement.id()));
    }
    ...
    private static void nineBlockStorageRecipes(
        ...
        ShapelessRecipeBuilder.shapeless(reverseCategory, baseItem, 9)
            ...
            .save(exporter, new ResourceLocation(reverseId));
        ShapedRecipeBuilder.shaped(compactingCategory, compactItem)
            ...
            .save(exporter, new ResourceLocation(compactingId));
    }
