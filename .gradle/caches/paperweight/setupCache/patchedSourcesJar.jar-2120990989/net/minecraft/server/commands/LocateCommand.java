...
public class LocateCommand {
    ...
    private static String getElementName(Pair<BlockPos, ? extends Holder<?>> result) {
        return result.getSecond().unwrapKey().map(key -> key.location().toString()).orElse("[unregistered]");
    }
    ...
    public static int showLocateResult(
        ...
        String string = predicate.unwrap().map(entry -> predicate.asPrintable(), tag -> predicate.asPrintable() + " (" + getElementName(result) + ")");
    ...
    public static int showLocateResult(
        ...
        String string = structure.unwrap().map(key -> key.location().toString(), key -> "#" + key.location() + " (" + getElementName(result) + ")");
        return showLocateResult(source, currentPos, result, successMessage, includeY, string, timeTaken);
    }
