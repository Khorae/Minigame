...
public class MacroFunction<T extends ExecutionCommandSource<T>> implements CommandFunction<T> {
    ...
    @Override
    public InstantiatedFunction<T> instantiate(@Nullable CompoundTag arguments, CommandDispatcher<T> dispatcher, T source) throws FunctionInstantiationException {
        if (arguments == null) {
            ...
            if (instantiatedFunction != null) {
                ...
                InstantiatedFunction<T> instantiatedFunction2 = this.substituteAndParse(this.parameters, list, dispatcher, source);
    ...
    private InstantiatedFunction<T> substituteAndParse(List<String> varNames, List<String> arguments, CommandDispatcher<T> dispatcher, T source) throws FunctionInstantiationException {
        ...
        for (MacroFunction.Entry<T> entry : this.entries) {
            ...
            list.add(entry.instantiate(list2, dispatcher, source, this.id));
        }
    ...
    interface Entry<T> {
        ...
        UnboundEntryAction<T> instantiate(List<String> args, CommandDispatcher<T> dispatcher, T source, ResourceLocation id) throws FunctionInstantiationException;
    }
    ...
    static class MacroEntry<T extends ExecutionCommandSource<T>> implements MacroFunction.Entry<T> {
        ...
        public MacroEntry(StringTemplate invocation, IntList variableIndices) {
        ...
        @Override
        public UnboundEntryAction<T> instantiate(List<String> args, CommandDispatcher<T> dispatcher, T source, ResourceLocation id) throws FunctionInstantiationException {
            String string = this.template.substitute(args);

            try {
                return CommandFunction.parseCommand(dispatcher, source, new StringReader(string));
            } catch (CommandSyntaxException var7) {
                throw new FunctionInstantiationException(
                    Component.translatable("commands.function.error.parse", Component.translationArg(id), string, var7.getMessage())
                );
            }
        }
    }
    ...
    static class PlainTextEntry<T> implements MacroFunction.Entry<T> {
        ...
        @Override
        public UnboundEntryAction<T> instantiate(List<String> args, CommandDispatcher<T> dispatcher, T source, ResourceLocation id) {
