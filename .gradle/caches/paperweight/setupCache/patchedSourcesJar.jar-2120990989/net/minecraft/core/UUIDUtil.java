...
public final class UUIDUtil {
    ...
    }, UndashedUuid::toString)).xmap(either -> either.map(uuid -> (UUID)uuid, uuid -> (UUID)uuid), Either::right);
    public static Codec<UUID> LENIENT_CODEC = Codec.either(CODEC, STRING_CODEC)
        .xmap(either -> either.map(uuid -> (UUID)uuid, uuid -> (UUID)uuid), Either::left);
