...
public record LocationPredicate(
    ...
    public boolean matches(ServerLevel world, double x, double y, double z) {
        if (this.position.isPresent() && !this.position.get().matches(x, y, z)) {
            ...
            return (!this.biome.isPresent() || bl && world.getBiome(blockPos).is(this.biome.get()))
                && (!this.structure.isPresent() || bl && world.structureManager().getStructureWithPieceAt(blockPos, this.structure.get()).isValid())
